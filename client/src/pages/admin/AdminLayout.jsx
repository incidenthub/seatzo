...
const AdminLayout = () => {
  ...
  return (
    <div className="flex min-h-screen bg-[#f8fafc] font-sans selection:bg-rose-500/30">
      <style>{`
        @import url('https://fonts.googleapis.com/css2?family=Syne:wght@700;800&family=Plus+Jakarta+Sans:wght@400;500;600;700&display=swap');
        * { font-family: 'Plus Jakarta Sans', sans-serif; }
        .nav-link-admin { display:flex; align-items:center; gap:12px; padding:12px 16px; border-radius:14px; font-size:14px; color:#64748b; text-decoration:none; transition:all 0.2s cubic-bezier(0.4, 0, 0.2, 1); font-weight: 500; }
        .nav-link-admin:hover { background:rgba(244,63,94,0.05); color:#f43f5e; }
        .nav-link-admin.active { background:white; color:#f43f5e; font-weight: 700; box-shadow: 0 4px 6px -1px rgb(0 0 0 / 0.1), 0 2px 4px -2px rgb(0 0 0 / 0.1); }
      `}</style>
      ...
      <aside className="w-72 bg-white border-r border-slate-200 flex flex-col p-6 fixed top-0 left-0 bottom-0 z-50">
        ...
        <div className="px-2 mb-10 group cursor-pointer" onClick={() => navigate("/")}>
          <div className="flex items-center gap-2">
            <div className="w-8 h-8 bg-rose-500 rounded-lg flex items-center justify-center shadow-[0_0_20px_rgba(244,63,94,0.3)]">
        ...
        <div className="mt-auto pt-6 border-t border-slate-100">
          <div className="flex items-center gap-3 p-3 bg-slate-50 rounded-2xl mb-4 border border-slate-100">
            ...
            <div className="flex-1 min-w-0">
              ...
              <p className="text-[10px] font-medium text-rose-500 uppercase tracking-wider">Super Admin</p>
            </div>
          </div>
          <button
            onClick={handleLogout}
            className="w-full flex items-center gap-3 px-4 py-3 rounded-xl text-sm font-bold text-slate-500 hover:text-rose-500 hover:bg-rose-50 transition-all group"
          >
